...
# Check if growthVB directory exists
if (!dir.exists("growthVB")) {
  cat("ERROR: growthVB directory not found in current working directory.\n")
  cat("Current directory:", getwd(), "\n")
  quit(status = 1)
}

# Generate documentation for the growthVB package
cat("Generating documentation for growthVB package...\n")

tryCatch(
  {
    # Document the package
    devtools::document("growthVB")
    cat("Documentation generated successfully!\n")

    # Verify documentation was created
    man_files <- list.files("growthVB/man", pattern = "\\.Rd$")
