...
#' enhanced posterior predictive checks, and model-specific growth diagnostics.
#'
...
plot_vb_bayes_diagnostics <- function(model,
  ...
  required_packages <- c("brms", "bayesplot", "posterior")
  ...
  create_diagnostics <- function(brms_model, sex_label = NULL) {
    ...
    if ("pp_checks" %in% diagnostic_types) {
      ...
      # PP check for mean
      pp_mean <- bayesplot::pp_check(brms_model, plotfun = "stat", stat = "mean", ndraws = ndraws) +
        ggplot2::labs(title = paste("PP Check - Mean", title_suffix))

      # PP check for standard deviation
      pp_sd <- bayesplot::pp_check(brms_model, plotfun = "stat", stat = "sd", ndraws = ndraws) +
        ggplot2::labs(title = paste("PP Check - Standard Deviation", title_suffix))

      # PP check for quantiles
      pp_quantiles <- bayesplot::pp_check(brms_model, plotfun = "stat_2d", stat = c("mean", "sd"), ndraws = ndraws) +
        ggplot2::labs(title = paste("PP Check - Mean vs SD", title_suffix))

      # Rootogram
      pp_rootogram <- try(
        {
          bayesplot::pp_check(brms_model, plotfun = "rootogram", ndraws = ndraws) +
    ...
    if ("residuals" %in% diagnostic_types) {
      # Get residuals
      residuals <- brms::residuals(brms_model, summary = FALSE)
      fitted_values <- brms::fitted(brms_model, summary = FALSE)

      # Residual scatter plot
      residual_data <- data.frame(
        fitted = apply(fitted_values, 2, mean),
        residual = apply(residuals, 2, mean),
        residual_sd = apply(residuals, 2, sd)
      )

      residual_scatter <- ggplot2::ggplot(residual_data, ggplot2::aes(x = fitted, y = residual)) +
        ...
        ggplot2::labs(
          title = paste("Posterior Mean Residuals vs Fitted", title_suffix),
          x = "Fitted Values", y = "Residuals"
        )

      # Standardised residuals
      std_residuals <- residuals / residual_data$residual_sd
      std_residual_data <- data.frame(
        fitted = residual_data$fitted,
        std_residual = apply(std_residuals, 2, mean)
      )

      std_residual_plot <- ggplot2::ggplot(std_residual_data, ggplot2::aes(x = fitted, y = std_residual)) +
        ggplot2::geom_point(alpha = 0.6, colour = "royalblue") +
        ggplot2::geom_hline(yintercept = 0, linetype = "dashed", colour = "red") +
        ggplot2::geom_hline(yintercept = c(-2, 2), linetype = "dotted", colour = "orange") +
        ggplot2::labs(
          title = paste("Standardised Residuals vs Fitted", title_suffix),
          x = "Fitted Values", y = "Standardised Residuals"
        )

      model_diagnostics$residuals <- list(
        residual_scatter = residual_scatter,
        standardised = std_residual_plot
      )
    }
    ...
    if ("growth_specific" %in% diagnostic_types) {
      ...
      if ("age" %in% names(original_data) && "length" %in% names(original_data)) {
        ...
        residuals <- brms::residuals(brms_model, summary = TRUE)
        age_residual_data <- data.frame(
          age = original_data$age,
          residual = residuals[, "Estimate"],
          residual_lower = residuals[, "Q2.5"],
          residual_upper = residuals[, "Q97.5"]
        )
        ...
        age_residual_plot <- ggplot2::ggplot(age_residual_data, ggplot2::aes(x = age, y = residual)) +
          ...
          ggplot2::geom_smooth(se = FALSE, colour = "black", method = "loess") +
          ggplot2::labs(
            title = paste("Residuals by Age", title_suffix),
            x = "Age", y = "Residuals"
          )
        ...
        predictions <- brms::posterior_predict(brms_model, newdata = new_data, summary = TRUE)
